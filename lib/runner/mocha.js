...
function MochaRunner(runnerConfig) {
  ...
  async function bindNemo(nemoCoreObject) {
    ...
    const session = await this.nemo.driver.getSession();
    this.currentTest.sessionId = session.id_;
    let testCircularSafe = prepareTestResult(this.currentTest);
    runnerConfig.emitter.emit('test:driver', testCircularSafe);
    return Promise.resolve();
  }
